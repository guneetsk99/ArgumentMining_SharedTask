...
def eval(model, dataset, args, output_results=False):
    ...
    if output_results:
            #sent=json.load(open('/content/drive/MyDrive/submissionrr2/RR-submission-v2/test.json'))
            sent=json.load(open('/content/drive/MyDrive/submissionrr2/RR-submission-v2/test2.json'))
            writer = Writer(args, all_preds, all_labels, all_lengths, all_sens_lengths, all_last_review_indice, all_bio_golds, all_bio_preds,sent)
...
def test(args):
    ...
    model_path = '/content/drive/MyDrive/X/your_model/BaselineLSTMModelpair.pt'

    model = torch.load(model_path).to(args.device)
    model.eval()

    sentence_packs = json.load(open('/content/drive/MyDrive/submissionrr2/RR-submission-v2/test2.json'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_bert_token', type=int, default=100,
    ...
    parser.add_argument('--cls_method', type=str, default='binary', choices=['binary', 'multiclass'],
                        help='relates to pairing classification')
    parser.add_argument('--model', type=str, default='BaselineLSTMModel', choices=['IterativeModel', 'IterativeAttnLossModel', 'ExternalRNNModel', 'JointModel', 'BaselineLSTMModel', 'BaselineBertModel', 'RNNModel', 'ExternalLSTMModel'],
                        help='model choice')
    ...
    parser.add_argument('--batch_size', type=int, default=1,
                        help='bathc size')
    parser.add_argument('--epochs', type=int, default=43,
